...
# 🔥 UPGRADE: DEFINITIVELY Replaced dead tvDatafeed with yfinance
...
HTML_TEMPLATE = """
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>AI Quant Dashboard</title><script src="https://cdn.tailwindcss.com"></script><script src="https://cdn.jsdelivr.net/npm/chart.js"></script><style>body { background-color: #0f172a; color: #f8fafc; font-family: 'Inter', sans-serif; } .glass-card { background: rgba(30, 41, 59, 0.7); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.1); }</style></head><body class="p-4 sm:p-6"><div class="max-w-md mx-auto"><div class="flex justify-between items-center mb-6"><div><h1 class="text-2xl font-bold text-emerald-400">V27.5 God Mode</h1><p class="text-xs text-slate-400">Approve/Reject Mode Active</p></div><div id="status-badge" class="px-3 py-1 rounded-full text-xs font-bold bg-emerald-500/20 text-emerald-400 border border-emerald-500/50">● ACTIVE</div></div><div class="grid grid-cols-2 gap-4 mb-6"><div class="glass-card p-4 rounded-xl text-center"><p class="text-xs text-slate-400 mb-1">Total PnL</p><p id="total-pnl" class="text-xl font-bold text-white">₹0.00</p></div><div class="glass-card p-4 rounded-xl text-center"><p class="text-xs text-slate-400 mb-1">Win Rate</p><p id="win-rate" class="text-xl font-bold text-blue-400">0%</p></div><div class="glass-card p-4 rounded-xl text-center"><p class="text-xs text-slate-400 mb-1">Total Trades</p><p id="total-trades" class="text-xl font-bold text-white">0</p></div><div class="glass-card p-4 rounded-xl text-center"><p class="text-xs text-slate-400 mb-1">Dynamic Capital</p><p id="dynamic-cap" class="text-xl font-bold text-purple-400">₹50K</p></div></div><h2 class="text-lg font-bold text-slate-300 mb-3">📈 Equity Curve</h2><div class="glass-card p-4 rounded-xl mb-6"><canvas id="equityChart" height="200"></canvas></div>
...
def process_single_symbol(sym, manual=False):
    ...
    if decision == "BUY 🟢" and cp <= prev_high: decision = "WAIT"
    if decision == "SELL 🔴" and cp >= prev_low: decision = "WAIT"
    ...
    if decision != "WAIT":
        ...
        # 🔥 NEW FEATURE: Approve/Reject Buttons Logic
        ...
        # We don't execute yet, just send buttons
        send_msg(AUTHORIZED_USER, msg, inline_kb)
        
        # Keep trade info in a pending state
        # (This dictionary would need to be persistent for a production semi-auto bot,
        # but for this example, we'll keep it in memory. It would lose state on Render restart)
        global pending_trades
        pending_trades[trade_id] = {
            "sym": sym, "final_decision": final_decision, "exec_price": exec_price, 
            "sl": sl, "tp": tp, "qty": qty, "features_str": features_str
        }

        return f"✅ {sym}: Trade Setup Found! Waiting for Approval." if manual else None
...
def process_command(chat_id, txt, cb_query_id=None):
    ...
    if txt.startswith("YES_"):
        ...
        if trade_id in pending_trades:
            ...
            send_msg(chat_id, f"✅ *APPROVED & EXECUTED!* \nSymbol: {t['sym']} entry triggered.")
            
            if cb_query_id:
                try: requests.post(f"{BASE_URL}/answerCallbackQuery", json={"callback_query_id": cb_query_id, "text": "Trade Approved!"})
                except: pass
        else:
        ...
        if trade_id in pending_trades:
            ...
            if cb_query_id:
                try: requests.post(f"{BASE_URL}/answerCallbackQuery", json={"callback_query_id": cb_query_id, "text": "Trade Rejected."})
                except: pass
        else:
            send_msg(chat_id, "❌ Trade session expired or already processed.")
        return
            ...
            if "result" in res:
                for upd in res["result"]:
                    ...
                    if "callback_query" in upd:
                        ...
                        # Process Approve/Reject data
                        process_command(chat_id, cb["data"], cb["id"])
                        
                        # Remove the keyboard after action
